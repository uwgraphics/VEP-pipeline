#  -*- coding: utf-8 -*-
# a slightly modified version of tei-decoder-stage designed to extract ONLY DIALOGUE from a TCP xml file
# use with the provided stage_config.yaml
__author__ = 'wchen', 'wintere'
...
prologue_list = ['prologue','verse prologue', 'prologue to drama', 'prologue to the king', 'prologue to the stage']

parser = argparse.ArgumentParser(description='TEI Decoder - Stage/Dialogue Extraction Edition')
parser.add_argument('yaml_config', help='path to config.yaml file - we recommend stage_config.yaml')
...
def node_process(node, ignore):
    ...
    if config[node.tag]['main-config'] is not None:
        if "csv" in config[node.tag]['main-config']:
            ...
            #elif node.tag not in csv_dict.keys(): # prevent from that key already exists
            else:
    ...
    if config[node.tag]['main-config'] is not None:
        ...
        if ignore is True:
            #ERIN: hard-coded prologue fix until we invent a more flexible system that can
            #ignore/traverse nodes by tag AND type
            #note that this is clumsy because the addition of text is not linear in the original code
            #ignored nodes are traversed and transcribed then set to '' after ANOTHER check that they are indeed ignored
            if node.tag == "FRONT":
                has_prologue = False
                for child in node.iter():
                    if 'TYPE' in child.attrib:
                        t = child.attrib['TYPE']
                        if t in prologue_list:
                            has_prologue = True
                            v_list, prologue_buf = node_process(child, False)
                            out_buf += ' ' + prologue_buf + '\n'
                if has_prologue == False:
                    out_buf = ''
            else:
                out_buf = ''
...
for dirpath, subdirs, files in os.walk(rootdir):
    for filename in files:
        if filename.endswith(".xml"): # only processing XML files
            ...
            while '  --  ' in str_to_write:
                str_to_write = str_to_write.replace('  --  ', ' -- ')
            # arrange the document to fit the limit of line length equal to 80
            buf = str_to_write
            str_to_write = ''
            line_len = 80
